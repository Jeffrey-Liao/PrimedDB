...
namespace liao::PrimedDB
{
	...
	{}
	Transaction::Transaction(const std::string& oprtor, const std::string& name, SQLType operation, int blockId, int location, unsigned size, UCharPtr memory)
	...
	{}
	Transaction::Transaction(const std::string& oprtor, const std::string& name,const std::string& str, bool neg)
	...
	Transaction::Transaction(const std::string& oprtor, const std::string& name, bool neg)
		:m_table(name), m_operator(oprtor)
	{
		std::string buffer;
	...
	{}
	void Transaction::operator=(Transaction&& move)noexcept
	{
	...
	SQLType Transaction::getType() const
	{
		return m_operation;
	}
	...
	const std::string& Transaction::getOperator()const
	{
		return m_operator;
	}
	const std::string& Transaction::getTable() const
	{
		return m_table;
	}
	...
	void Transaction::setBlockId(int newBlockId)
	{
		m_blockId = newBlockId;
	}
	int Transaction::getLocation() const
	{
		return m_location;
	}
	unsigned Transaction::size() const
	{
		return m_size;
	}
	...
	SCharPtr Transaction::getMemory()
	{
		return m_memory;
	}
	void Transaction::setValid()
	{
		m_valid = true;
	}
	void Transaction::setInvalid()
	{
		m_valid = false;
	}
	...
	void Transaction::fromString(const std::string& str)
	{
	...
	void Transaction::negFromString(const std::string& str)
	{
		std::vector<std::string> vec;
		StaticFunc::Split(vec, str, ':');
		if (vec.size() == 7)
